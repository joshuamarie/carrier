...
`{carrier}` ships as a native binary. There is no R package to install for the CLI itself, only for the `{box}` fork it depends on at runtime (see [below](#the-box-fork)).

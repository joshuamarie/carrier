...
The practical difference from classic R packages is explicit: You are not forced to put native sources in a directory named `src/`. Any directory name is fine, as long as `{carrier}` can find it, either by automatic discovery (this is `src/` by default) or by an explicit `[native]` entry (see [The `[native]` block](#native-block)).

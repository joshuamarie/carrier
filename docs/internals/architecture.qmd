...
└── carrier-core/      # lib crate — all real logic, reusable outside the CLI
    └── src/
        ...
        ├── formats/          # tar.gz and .rmbx archive read/write
...
Every bundled archive (`.tar.gz` or `.rmbx`) embeds a `manifest.json` built from `carrier.toml` by `ops::bundle::build_manifest`:
...
`carrier install` reads this manifest back out of `.rmbx`/`.tar.gz` archives instead of re-parsing `carrier.toml`, so the archive is self-describing.

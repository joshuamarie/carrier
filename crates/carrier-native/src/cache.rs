...
/// Hash the contents of `native_dir`. Every file's path relative to
/// it, plus its bytes, in sorted order for determinism. This is the
/// cache key that decides whether a previously-built artifact can be
/// reused instead of recompiling: same source bytes, same platform,
/// same R version => same output, so `R CMD SHLIB` never has to run

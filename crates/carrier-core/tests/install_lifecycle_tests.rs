...
fn install_from_dir_then_remove_round_trip() {
    ...
    install::run(project.path().to_str().unwrap(), false).unwrap();
    ...
    // carrier.toml is a project manifest, not part of the installable
    // module — it must not end up in the installed tree.
...
fn install_errors_on_project_without_carrier_toml() {
    ...
    let err = install::run(project.path().to_str().unwrap(), false).unwrap_err();
